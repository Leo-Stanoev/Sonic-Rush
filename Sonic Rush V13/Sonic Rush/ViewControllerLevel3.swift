...
class ViewControllerLevel3: UIViewController, ReleaseDelegate3, UpdateAngle3{
    ...
    func PlayBGMusic(){
        ...
        func CreateObstacle(){
            ...
            let obsframe = CGRect(x:w*randomObstaclePosX,y:h*randomObstaclePosY,width: w*0.2,height: h*0.5)
            obstacle = UIView(frame: obsframe)
            obstacle?.center = CGPoint(x:w*randomObstaclePosX,y:h*randomObstaclePosY)
            obstacle?.layer.borderColor = UIColor.red.cgColor
            obstacle?.layer.borderWidth = 6
